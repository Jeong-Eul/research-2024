...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Vital mapping
        self.vital_mapping = nn.Linear(self.seq_len * self.d_ff, self.d_ff)
        ...
        self.output_projection = ClassificationHead(self.d_ff, 1, head_dropout=0)
    ...
    def forecast(self, x_enc, time, real_time, demo, null_mask):
        ...
        for b in range(B):
            ...
            for n in range(N):
                ...
                prompt_batch = (
                    ...
                    f"the trend of input is {result}<|end_prompt|>"
                )
        ...
        nan_mask = torch.isnan(null_mask[:, :, :N_vital])
        ...
        lab_mask = (x_lab != 0)& (~torch.isnan(x_lab))
        ...
        lab_processed = torch.einsum('bnt,bndt->bnd', x_lab.permute(0, 2, 1).float(), time_embedding_with_ps.permute(0, 1, 3, 2))
        scale = 1. / torch.sqrt(torch.tensor(lab_processed.shape[-1]))
        lab_processed = lab_processed * scale
        
        # Variable attention
        
        maped_vital = self.vital_mapping(out.reshape(-1, T * self.d_ff)) # B*N_vital, D
...
class Variable_Attention(nn.Module):
    ...
    def forward(self, target_embedding):
        ...
        beta_cal = beta_ori.permute(0, 2, 1)
        out = torch.einsum("bnhk,bnh->bhk", V, beta_cal) # B, H, E

        out = self.out_projection_layer(out.view(B, -1))
