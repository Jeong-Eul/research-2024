...
from model import TimeLLM, EHRTime_LLM, EHRTime_LLM_v4
...
from utils.tools import del_files, EarlyStopping, adjust_learning_rate, vali, load_content, load_vocabulary, load_domain_content, vali_v4
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patch_len', type=int, default=8, help='dimension of fcn')
    parser.add_argument('--stride', type=int, default=3, help='dimension of fcn')
    ...
    for ii in range(args.itr):
        ...
        if args.model == 'TimeLLM':
            ...
            model = EHRTime_LLM_v4.Model(args).float()
        ...
        if args.lradj == 'COS':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(model_optim, T_max=15, eta_min=1e-8)
        else:
        ...
        for epoch in range(args.train_epochs):
            ...
            for i, (batch_x, batch_x_mask, batch_delta, batch_y, batch_time, batch_real_time, batch_demo, batch_vital_mean, batch_mask) in tqdm(enumerate(train_loader)):
                with accelerator.accumulate(model):
                    iter_count += 1
                    # model_optim.zero_grad()
              
                    batch_x = batch_x.float().to(accelerator.device)
                    batch_x_mask = batch_x_mask.float().to(accelerator.device)
                    batch_delta = batch_delta.float().to(accelerator.device)
                    batch_y = batch_y.float().to(accelerator.device).unsqueeze(1)
                    batch_time = batch_time.float().to(accelerator.device)
                    batch_vital_mean = batch_vital_mean.float().to(accelerator.device)
                    batch_mask = batch_mask.float().to(accelerator.device)
            
                    if args.use_amp:
                        with accelerator.autocast():
                            if args.output_attention:
                                outputs = model(batch_x, batch_x_mask, batch_delta, batch_time, batch_real_time, batch_demo, batch_vital_mean, batch_mask)[0]
                            else:
                                outputs = model(batch_x, batch_x_mask, batch_delta, batch_time, batch_real_time, batch_demo, batch_vital_mean, batch_mask)
            ...
            vali_loss = vali_v4(args, accelerator, model, vali_data, vali_loader, criterion)
