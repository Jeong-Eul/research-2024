    ...
    def forecast(self, x_enc, time, real_time, demo, null_mask):
        ...
        for b in range(B):
            ...
            for n in range(N):
                ...
                prompt_batch = (
                    f"Who you are: You are an ICU doctor tasked with estimating vital sign values that were not recorded during patient monitoring. "
                    f"Task Description: Your goal is to predict the missing values marked by [MASK] based on the surrounding time points. Use your understanding of vital sign trends and patient history to provide a complete time series embedding."
                    ...
                    f"Missing values occurred at time points: {missing_time_points}, and these were replaced with [MASK]."
                )
        ...
        input_ids = self.tokenizer.encode('[MASK]', return_tensors='pt')
        mask_token = self.llm_model.get_input_embeddings()(input_ids.to(x_enc.device))
        mask_index = (input_ids[0] == self.tokenizer.mask_token_id).nonzero(as_tuple=True)[0]
        mask_token = mask_token[0, mask_index]
        ...
        # Concatenate
        total_out = torch.cat((prompt_embeddings, rep_out.to(x_enc.device)), dim=1) # B*N_vital, prompt + Time, d_ll
