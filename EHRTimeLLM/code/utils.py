...
def evaluate(accelerator, model, P_tensor, P_time_tensor, P_static_tensor, mask, batch_size=4, n_classes=2, static=1):
    model.eval()
...
#     N, T, Ff = P_tensor.shape
#     n_batches, rem = N // batch_size, N % batch_size
#     out = torch.zeros(N, n_classes).to(accelerator.device)  # 결과 텐서 초기화
#     start = 0
...
def evaluate_standard(accelerator, model, P_tensor, P_time_tensor, P_static_tensor, mask, batch_size=4, n_classes=2, static=1):
    model.eval()
    P_tensor = P_tensor[:, :, :int(P_tensor.shape[2] / 2)].to(accelerator.device)
    P_time_tensor = P_time_tensor.to(accelerator.device) 
    mask = mask.to(accelerator.device)
