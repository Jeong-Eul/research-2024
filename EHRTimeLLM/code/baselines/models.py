...
class GRUD(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, x_mean=0,
        ...
        self.zeros = nn.Parameter(torch.zeros(input_size), requires_grad=False)
        ...
        with torch.cuda.device_of(any_param):
            ...
            with torch.no_grad():
                torch._cudnn_rnn_flatten_weight(
                    ...
                    self.input_size, rnn.get_cudnn_mode(self.mode), self.hidden_size, int(self.num_layers),
    ...
    def forward(self, input, dataset_name='P12'):
        ...
        output = None
        h = Hidden_State.to(X.device)

        # decay rates gamma
        w_dg_x = getattr(self, 'weight_dg_x').to(X.device)
        w_dg_h = getattr(self, 'weight_dg_h').to(X.device)

        # z
        w_xz = getattr(self, 'weight_xz').to(X.device)
        w_hz = getattr(self, 'weight_hz').to(X.device)
        w_mz = getattr(self, 'weight_mz').to(X.device)

        # r
        w_xr = getattr(self, 'weight_xr').to(X.device)
        w_hr = getattr(self, 'weight_hr').to(X.device)
        w_mr = getattr(self, 'weight_mr').to(X.device)

        # h_tilde
        w_xh = getattr(self, 'weight_xh').to(X.device)
        w_hh = getattr(self, 'weight_hh').to(X.device)
        w_mh = getattr(self, 'weight_mh').to(X.device)

        # bias
        b_dg_x = getattr(self, 'bias_dg_x').to(X.device)
        b_dg_h = getattr(self, 'bias_dg_h').to(X.device)
        b_z = getattr(self, 'bias_z').to(X.device)
        b_r = getattr(self, 'bias_r').to(X.device)
        b_h = getattr(self, 'bias_h').to(X.device)
        ...
        for layer in range(self.num_layers):
            ...
            m = torch.squeeze(Mask[:, layer:layer + 1]).to(X.device)
            d = torch.squeeze(Delta[:, layer:layer + 1]).to(X.device)

            # (4)
            gamma_x = torch.exp(-torch.max(self.zeros.to(X.device), (w_dg_x.to(X.device) * d + b_dg_x.to(X.device))))
            gamma_h = torch.exp(-torch.max(self.zeros.to(X.device), (w_dg_h.to(X.device) * d + b_dg_h.to(X.device))))
            ...
            # (6)
            if self.dropout == 0:
                h = gamma_h.to(X.device) * h.to(X.device)
        ...
        w_hy = getattr(self, 'weight_hy').to(X.device)
        b_y = getattr(self, 'bias_y').to(X.device)
