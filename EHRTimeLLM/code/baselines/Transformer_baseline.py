    ...
    for missing_ratio in missing_ratios:
        ...
        for k in range(n_splits):
            ...
            if wandb:
                ...
                # config = wandb.config
                wandb.init(name=f'Transformer_mean-split-'+ str(split_idx),
            ...
            for m in range(n_runs):
                ...
                for epoch in range(num_epochs):
                    ...
                    for n in range(n_batches):
                        ...
                        if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
                            P, Ptime, Pstatic, y = Ptrain_tensor[:, idx, :].to(accelerator.device), Ptrain_time_tensor[:, idx].to(accelerator.device), \
                                                Ptrain_static_tensor[idx].to(accelerator.device), ytrain_tensor[idx].to(accelerator.device)
                        elif dataset == 'PAM':
                            P, Ptime, Pstatic, y = Ptrain_tensor[:, idx, :].to(accelerator.device), Ptrain_time_tensor[:, idx].to(accelerator.device), \
                        ...
                        outputs = evaluate_standard(accelerator, model, P, Ptime, Pstatic, static=static_info)
                    ...
                    if epoch ==0 or epoch % 1 == 0:
                        with torch.no_grad():
                            out_val = evaluate_standard(accelerator, model, Pval_tensor, Pval_time_tensor, Pval_static_tensor, static=static_info)
                ...
                with torch.no_grad():
                    out_test = evaluate(accelerator, model, Ptest_tensor, Ptest_time_tensor, Ptest_static_tensor, n_classes=n_classes, static=static_info).numpy()
