...
def evaluate(accelerator, model, P_tensor, P_time_tensor, P_static_tensor, batch_size=4, n_classes=2, static=1):
    model.eval()
    P_tensor = P_tensor.to(accelerator.device)
...
def evaluate_standard(accelerator, model, P_tensor, P_time_tensor, P_static_tensor, batch_size=4, n_classes=2, static=1):
    P_tensor = P_tensor.to(accelerator.device)
    P_time_tensor = P_time_tensor.to(accelerator.device)
    if static is None:
